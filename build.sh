...
then
    ...
    if [ "$RELEASE" == "alpha" ]
    then
	for dev in mako grouper maguro i9100 i9100g i9300 n7000 n7100 manta find5 yuga odin jfltecan jfltetmo jfltespr jflteusc jfltevzw jflteatt n8000 n8013 jfltexx ; do
		export RV_PRODUCT=$dev
		cd ~/revolt
		android-build -C -v $ver -o $outdir revolt_$dev-userdebug
		echo -e "ReVolt Compilation Finished for $dev"
	...
	for dev3 in m7ul m7att m7tmo m7spr ; do
		export RV_PRODUCT=$dev3
		cd ~/revolt
		rm -rf hardware/cm
		android-build -C -v $ver -o $outdir revolt_$dev3-userdebug
		echo -e "ReVolt Compilation Finished for $dev3"
	        if [ $? -eq 0 ]; then
	                echo "ALPHA Build Successfully done for $dev3">>log.txt
			ncftpput -f login.cfg /$dev3/ALPHA/ $outdir/revolt_$dev3-$ver.zip
			scp $outdir/revolt_$dev3-$ver.zip johnhany97@upload.goo.im:~/public_html/ReVolt_JB_$dev3/ALPHA/
			rm -rf $outdir/revolt_$dev3-$ver.zip
		else
		        echo "ALPHA Build FAILED for $dev3">>/raid/johnhany97/log.txt
		fi
	done
    else
	for dev in mako grouper maguro i9100 i9100g i9300 n7000 n7100 manta find5 yuga odin m7ul m7att m7tmo m7spr jfltecan jfltetmo jfltespr jflteusc jfltevzw jflteatt n8000 n8013 jfltexx ; do
